class PagesController < ApplicationController
  def create_csv
    if !params[:title_team_player].blank? && !params[:input].blank?
      sku = params[:royalty_sku] unless params[:royalty_sku].blank?
      uploaded_ttp_path, uploaded_inputs_path = Uploader.call(params[:title_team_player].path, params[:input].path)
      SendCsvJob.perform_later(User.find(session[:user_id]).email, uploaded_ttp_path, uploaded_inputs_path, sku)
